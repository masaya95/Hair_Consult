...
<div class="container">
  <div class="row">
    ...
    <div class="col-md-8">
      ...
      <div class="card custom-card">
        <div class="card-header">
          <div class="rounded bg-warning text-center font-weight">{{ $auth->name }}のいいねした記事</div>
        ...
        @if(count($postlikes) > 0)
          @foreach($postlikes as $postlike)
            <div class="card custom-card">
              <div class="card-header">
                <span class="rounded bg-warning category-icon">{{ $postlike->post->category }}</span>
              </div>
              <div class="card-body">
                <h4 class="card-title">
                  <a href="/post/{{ $postlike->post->id }}" class="alert-link">{{ $postlike->post->title }}</a>
                </h4>
                <div class="box">
                  <p class="card-text">
                    {{ $postlike->post->body }}
              ...
              <div class="card-footer">
                <div class="text-right">コメント📣({{ $postlike->post->comments->count() }})　いいね✌️({{ $postlike->post->good_btn }})</br>
                <span class="text-muted">投稿時間：{{ $postlike->post->created_at }}</span></div>
              </div>
            </div>
          @endforeach
          {{ $postlikes->links() }}
