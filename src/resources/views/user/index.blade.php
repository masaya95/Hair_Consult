...
<div class="container">
  <div class="row">
    ...
    <div class="col-md-8">
      ...
      <div class="card custom-card">
        <div class="card-header">
          <div class="rounded bg-warning text-center font-weight">{{ $auth->name }}の投稿記事</div>
        ...
        <div class="card-body">
          @if(count($posts) > 0)
            @foreach($posts as $post)
              <div class="card custom-card">
                <div class="card-header">
                  <span class="rounded bg-warning category-icon">{{ $post->category }}</span>
                </div>
                <div class="card-body">
                  <h4 class="card-title">
                    <a href="/post/{{ $post->id }}" class="alert-link">{{ $post->title }}</a>
                  </h4>
                  <div class="box">
                    <p class="card-text">
                      {{ $post->body }}
                ...
                <div class="card-footer">
                  <div class="text-right">コメント📣({{ $post->comments->count() }})　いいね✌️({{ $post->post_likes->count() }})</br>
                  <span class="text-muted">投稿時間：{{ $post->created_at }}</span></div>
                  <button><a href="/post/edit/{{ $post->id }}">編集する</a></button>
                  <button><a href="/post/{{ $post->id }}/delete">削除する</a></button>
                </div>
              </div>
            @endforeach
            {{ $posts->links() }}
