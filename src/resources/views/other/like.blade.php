...
<div class="container">
  <div class="row">
    ...
    <div class="col-md-8">
      <!-- 内容 -->
      <div class="custom-main-var rounded bg-primary text-center text-nowrap"><div class="box-center text-white font-weight text-nowrap">{{ $other->name }}さんのページ</div></div>
      ...
      <div class="card custom-card">
        <div class="card-header">
          <div class="rounded bg-warning text-center font-weight">{{ $other->name }}のいいねした記事</div>
        ...
        <div class="card-body">
          @if(count($other_post_likes) > 0)
            @foreach($other_post_likes as $other_post_like)
              <div class="card custom-card">
                <div class="card-header">
                  <span class="rounded bg-warning category-icon">{{ $other_post_like->post->category }}</span>
                </div>
                <div class="card-body">
                  <h4 class="card-title">
                    <a href="/post/{{ $other_post_like->post->id }}" class="alert-link">{{ $other_post_like->post->title }}</a>
                  </h4>
                  <div class="box">
                    <p class="card-text">
                      {{ $other_post_like->post->body }}
                    </p>
                  </div>
                </div>
                <div class="card-footer">
                  <div class="text-right">コメント📣({{ $other_post_like->post->comments->count() }})　いいね✌️({{ $other_post_like->post->post_likes->count() }})</br>
                  <span class="text-muted">投稿時間：{{ $other_post_like->post->created_at }}</span></div>
                </div>
              </div>
            @endforeach
