...
<div class="container">
  <div class="row">
    ...
    <div class="col-md-8">
      ...
      <div class="card custom-card">
        <div class="card-header">
          <div class="rounded bg-warning text-center font-weight">{{ $other->name }}のコメントした記事</div>
        ...
        <div class="card-body">
          @if(count($other_comments) > 0)
            @foreach($other_comments as $other_comment)
              <div class="card custom-card">
                <div class="card-header">
                  <span class="rounded bg-warning category-icon">{{ $other_comment->post->category }}</span>
                </div>
                <div class="card-body">
                  <h4 class="card-title">
                    <a href="/post/{{ $other_comment->post->id }}" class="alert-link">{{ $other_comment->post->title }}</a>
                  </h4>
                  <div class="box">
                    <p class="card-text">
                      {{ $other_comment->post->body }}
                    </p>
                  </div>
                </div>
                <div class="card-footer">
                  <div class="text-right">コメント📣({{ $other_comment->post->comments->count() }})　いいね✌️({{ $other_comment->post->post_likes->count() }})</br>
                  <span class="text-muted">投稿時間：{{ $other_comment->post->created_at }}</span></div>
                </div>
              </div>
            @endforeach
